...
3. [Geryon] captions scroll underneath; the browser speaks each line in sync (Web Speech API). No audio file. If speech isn’t available, captions still play. Occasional dream-flashes — dense colored glyph-particle bursts, 8–36ms — lock to utterance start (caption reveal if speech is off), a late softened morph-peak, and the Turn smash, then dissolve back into the crab. Never photos. `prefers-reduced-motion` skips them.
